...
class User {
  ...
  static login = async (req, res) => {
    try {
      let userData = await userModel.loginUser(
        req.body.email,
        req.body.password
      );
      ...
      console.log(role);
      const data = {userData, permissions: role.permissions };
      console.log(data);
  ...
  static allUsers = async (req, res) => {
    try {
      // await req.user.populate("myUrls")
  ...
  static addAddr = async (req, res) => {
    try {
      req.user = await userModel.findByIdAndUpdate(req.user.id, req.body);
      await req.user.save();
